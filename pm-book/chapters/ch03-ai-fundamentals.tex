...
Think of this as the mental model chapter. Once you understand how these systems actually work, their behavior becomes predictable. Their failures make sense. Their value becomes clear.

\noindent (Insert Figure: AI Decision Framework---What PMs Need to Know)
...
\begin{insightbox}{Key Insight: Plausible vs. True}
The single most important thing to understand about LLMs: they generate \textbf{plausible} outputs, not \textbf{true} outputs. A response can be fluent, confident, and completely wrong. The model doesn't know the difference. You have to.
\end{insightbox}
...
CMR's research on strategic AI tasks captures this limitation precisely: AI struggles with ``tasks requiring multi-step reasoning and human behavioral understanding'' [S1]. A good PM challenges requirements, questions assumptions, and identifies edge cases. AI will do what you ask. That's a feature and a bug. It makes AI useful for execution, but it means you need to provide the judgment layer.
...
The NIST AI Risk Management Framework identifies this as a core reliability risk: models can produce ``outputs that are plausible but factually incorrect'' [S30]. The causes are structural. Models are optimizing for plausible outputs, not true outputs. If a plausible response includes a made-up fact, the model will generate it without hesitation. It doesn't know it's making something up.

\begin{insightbox}{Key Insight: Hallucination Is a Feature, Not a Bug}
Hallucination isn't a temporary flaw that will be fixed in the next version. It's a structural consequence of how these models work. They're pattern-completion engines, not truth-verification engines. Plan accordingly. Build verification into your workflow, not as an afterthought, but as a core step.
\end{insightbox}

\noindent (Insert Table: Types of Hallucinations and Mitigation Strategies)
...
Every AI decision is a trade-off decision. When you're deciding whether to use AI for a task, run through a quick mental checklist. What do you gain? Usually speed, coverage, or variety in options. What do you risk? Typically accuracy, nuance, or originality. What's the verification cost? How much time will you spend checking outputs? And what happens if it's wrong? What are the stakes of an undetected error?

For low-stakes, high-volume tasks, AI is almost always worth it. The verification cost is low, the consequences of errors are minimal, and the speed gains compound. For high-stakes, nuanced decisions---anything customer-facing, strategic, or legally significant---AI is a starting point that requires significant human judgment.
...
\centering
\figTaskMatrix
\caption{AI Task Suitability Matrix: Match your verification effort to the stakes and volume of the task. High-volume, low-stakes work (drafts, brainstorms) can use AI heavily with light verification. High-stakes work requires rigorous human review regardless of volume.}
...
The fundamentals for PMs come down to five principles. First, models generate plausible outputs, not true outputs---so verification isn't optional. Second, every AI decision is a trade-off between speed, accuracy, and verification cost. Third, better prompts get better outputs, and prompting is a skill worth developing. Fourth, AI can propose but it can't decide---the judgment is still yours. Fifth, leverage is earned through verification, not assumed through adoption.
...
\begin{insightbox}{Key Insight: The Verification Principle}
Leverage is earned through verification. That's the principle. That's the practice. That's how you use AI without outsourcing your judgment. The teams that get value from AI aren't the ones that trust it most---they're the ones that verify it systematically.
\end{insightbox}

With this foundation in place, the next chapter shows you how to put these principles into practice. We'll walk through specific workflows where AI adds real value---and the verification habits that make that value stick.
